...
public class AdapterForAds extends PagerAdapter {
    private ArrayList<HashMap<String, String>> images;
    private LayoutInflater inflater;
    private Context context;
    List<AdsData> adsData;

    public AdapterForAds(FragmentActivity activity, List<AdsData> adsData) {
    ...
    public Object instantiateItem(ViewGroup view, int position) {
      ...
      /*  Picasso.with(context).load(arrayList.get(position))
       ...
       /* myImage.setOnClickListener(new View.OnClickListener() {
        ...
        Picasso.with(context).load(adsData.get(position).getImage()).fit()
                .placeholder(R.drawable.logo)
                .error(R.drawable.logo).into(myImage);
        /*Picasso.with(context).load(adsData.get(position).getImage()).placeholder(R.drawable.placeholder).into(new Target() {
